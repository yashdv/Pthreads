...
/*
   ...
   2) input is from command line. the format is "./a.out <file1> <file2> <file3> ... <file n> <destination path>" qoutes for clarity. Other formats are not allowed
   3) n < 100
   4) number of threads created to do task cannot be altered by any input from user
*/
...
sem_t sem[100],sem2;
int block_cnt[100] = {0}, size[100], blockdone[100] = {0}, whichfile, ARGC;
char name[100][100], dest[100];
FILE **fc;
...
void *copyx(void *arg)
{
	...
	FILE *fp;
	char c;
	int i,byte = blocksz, myblock = inf->whichblock, myfile = inf->fileno, shift = (myblock-1)*blocksz;
...
{
	int i,sumblock = 0,whichblockmain,whichfilemain;
	FILE *fp;
	struct info inf[100];
	char t[100];
